...
// 랜덤 추출할 루틴 정보
const getMockData = async (formData) => {
    // 사용자가 선택한 반복일수
    const dayRepeat = formData.dayRepeat;

    try {
        const response = await apiAxios.get('/exercise/routine', {
            params: { dayRepeat: dayRepeat },
        });
        console.log('랜덤 추출 데이터 :: ', response.data);
        return response.data;
    } catch (error) {
        console.error('운동 루틴 추출 실패', error);
        return [];
    }
    // const exerciseList = [
...
// 공공데이터 정보
const fetchAllExerciseOpenData = async () => {
    try {
        const response = await apiAxios.get('/exercise/openData');
        // console.log('운동 공공데이터 추출 성공 :: ', response.data);
        return response.data;
    } catch (error) {
        console.error('운동 공공데이터 추출 실패', error);
        return [];
    }
};

export { getMockData, fetchAllExerciseOpenData };
