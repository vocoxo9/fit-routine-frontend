...
function RecommendExercise() {
    ...
    useEffect(() => {
        ...
        mockData.forEach((oneDayData) => {
            initialCheckedData[oneDayData.dayNo]
                = oneDayData.exerciseList.map((ex) => ex.id);
        });
    ...
    const handleCheckboxClick = (dayNo, exerciseId, isChecked) => {
        setCheckedItems(prev => {
            ...
            if (isChecked) {
                ...
                return {
                    ...prev,
                    [dayNo]: currentCheckedItems.filter(id => id !== exerciseId)
                };
            }
        });
    }
                            ...
                            <div className={styles.category}>
                                {categoryList.map((category, idx) => (
                                    <Category
                                        key={idx}
                                        text={category}
                                        onClick={() => handleShowCheckList(dayNo, category)}
                                        isSelected={selectCategory[dayNo] === category}
                                    />
                                ))}
                            </div>
                        )}
